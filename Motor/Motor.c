...
const Timer_A_UpModeConfig upConfigMotor =
{
...
int motorMain(void)
 {
    ...
    Timer_A_configureUpMode(TIMER_A1_BASE, &upConfigMotor);
...
float getPIDOutputLeft()
{
    ...
    //error = (double) (desiredNotches - rightNotchesDetected) / (double)desiredNotches;
    //error = (rightNotchesDetected-  desiredNotches);
    ...
    printf(" Error = %.2f\t DC Left = %d\t PID Left = %.2f\n",error,pwmConfig2.dutyCycle,pidOutput);
...
float getPIDOutputRight()
{
    ...
    //printf("Output Left = %d\t Output Right = %d\t DC Right = %d\n", leftNotchesDetected,rightNotchesDetected ,pwmConfig.dutyCycle);
